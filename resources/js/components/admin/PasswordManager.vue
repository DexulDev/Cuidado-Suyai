...
export default {
  ...
  methods: {
    validateForm() {
      ...
      // Mostrar advertencia para contraseñas débiles pero permitirlas
      if (this.passwordStrength < 3 && this.form.new_password.length >= 5) {
        this.showWeakPasswordModal();
        return true; // Permitir continuar aunque la contraseña sea débil
      }
    ...
    async submitForm() {
      ...
      // Aceptar contraseña débil pero mostrar advertencia si corresponde
      if (this.passwordStrength < 3 && this.form.new_password.length >= 5) {
        this.showWeakPasswordModal();
      } else {
        // Si la contraseña es fuerte, continuar directamente
        this.submitFormFinal();
      }
    },
    ...
    async submitFormFinal() {
      ...
      try {
        // Enviar directamente la solicitud usando la URL proporcionada
        const requestData = {
        ...
        if (response.ok) {
          ...
          if (data.errors) {
            ...
            this.formErrors = ['Error al actualizar la contraseña'];
          }
        }
      } catch (error) {
        console.error('Error en la actualización de contraseña:', error);
        this.formErrors = ['Ocurrió un error de conexión al actualizar la contraseña'];
      } finally {
        setTimeout(() => {
